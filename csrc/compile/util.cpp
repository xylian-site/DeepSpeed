...
std::string tensorToString(
    const at::Tensor& t, size_t max_elem, size_t max_str_len) {
  ...
  if (t.scalar_type() == c10::ScalarType::Half ||
      t.scalar_type() == c10::ScalarType::BFloat16) {
...
std::string tensorPtrToString(
    void* ptr, size_t size, c10::ScalarType datatype, size_t max_elem,
    size_t max_str_len) {
  ...
  if (datatype == c10::ScalarType::Long) {
    ...
    return tensorPtrToString(static_cast<float*>(ptr), elem_size, max_str_len);
      ...
      datatype == c10::ScalarType::Half ||
      datatype == c10::ScalarType::BFloat16) {
    ...
    return tensorPtrToString(
        (float*)float_ten.data_ptr(), elem_size, max_str_len);
  }
  std::stringstream ss;
  ss << "Failed to convert tensor ptr to string. Invalid type of tensor: "
     << toString(datatype);
  throw std::invalid_argument(ss.str());
}
}