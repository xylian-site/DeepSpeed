...
def add_offload_parameter(graph_id: int, gm: GraphModule, node: Node, ds_id: int):
    ...
    with gm.graph.inserting_after(node):
        ...
        for a in [graph_id, ds_id]:  # To add ds_id
            args += (a, )
        new_node = gm.graph.create_node('call_function',  torch.ops.dc.offload_parameter.default, args, {},name="offload_parameter")
...
def add_reload_parameter(graph_id: int, gm: GraphModule, node: Node, ds_id: int):
    ...
    with gm.graph.inserting_after(node):
        ...
        for a in [graph_id, ds_id]:  # To add ds_id
            args += (a, )
        new_node = gm.graph.create_node('call_function',  torch.ops.dc.reload_parameter.default, args, {}, name=f"reload_parameter")
    return new_node
