...
from ..fx import add_postprocess, _make_node_meta, get_output_node, move_primals_to_head, clone_graph
from ..profilers.graph_profile import ProfilingInterpreter
from ..list_schedule import geedy_no_comm_schedule
...
def add_z3_gather_release_fw(gm: GraphModule,
    ...
    profile_graph = clone_graph(gm.graph)

    real_inputs = create_inputs_fn()
    param_indices = profiling_results[graph_id].param_indices

    profile_graph = add_gather_and_release(graph_id, profile_graph, param_manager[graph_id],
                                           get_param_nodes(profile_graph, param_indices))

    nz3.register_graph_z3(graph_id, [v[1] for v in param_indices])  # Need this before profiling

    gm.graph = profile_graph

    def run_profile(graph_mod):
        profiler = ProfilingInterpreter(graph_mod, debug_log=debug_log)
    ...
    reordered_graph = geedy_no_comm_schedule(
        original_graph,
        profile_graph,
        ...
        bwd=False,
        debug_log=debug_log)
    reordered_graph = add_gather_and_release(graph_id, reordered_graph, param_manager[graph_id],
                                             get_param_nodes(reordered_graph, param_indices))
    gm.graph = reordered_graph
    run_profile(gm)
...
def add_z3_gather_release_bw(gm: GraphModule,
    ...
    param_nodes_bw, param_name_to_grad = param_manager[graph_id].get_bwd_mapping(profile_graph)
    gm.graph = add_gather_and_reduce(graph_id, profile_graph, param_manager[graph_id], param_nodes_bw,
                                     param_name_to_grad)

    def run_profile(graph_mod):
        input_nodes = get_input_nodes(graph_mod.graph)
        real_inputs = create_inputs_fn()
        assert len(input_nodes) == len(real_inputs), f"Expected {len(real_inputs)} inputs, got {len(input_nodes)}"
        real_outputs = ProfilingInterpreter(graph_mod, debug_log=debug_log).run(*real_inputs)
        del real_outputs
        gc.collect()
        get_accelerator().empty_cache()
    ...
    reordered_graph = geedy_no_comm_schedule(
        ...
        bwd=True,
        debug_log=debug_log)
    param_nodes_bw, param_name_to_grad = param_manager[graph_id].get_bwd_mapping(reordered_graph)
    gm.graph = add_gather_and_reduce(graph_id, reordered_graph, param_manager[graph_id], param_nodes_bw,
                                     param_name_to_grad)
    run_profile(gm)
